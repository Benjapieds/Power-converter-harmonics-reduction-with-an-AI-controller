...
if __name__ == "__main__":
    ...
    repeats = B // base_B
    if B % base_B != 0:
        raise ValueError("B must be a multiple of 3 to repeat base values exactly")

    I_refs = torch.tensor([10.0, 20.0, 30.0], dtype=torch.float32).repeat(repeats)

    inputs = generate_input_sequence_torch(I_refs)
    B, C_resolution = inputs.shape
    ...
    for i in range(B):
        plt.plot(inputs[i].cpu().numpy(), label=f"Reference {i+1}")
    plt.title(f"Reference Signal")
    plt.xlabel("Time Steps (20kHz)")
    ...
    initialisation = generate_initialization_torch(B)
    print(f"Initialisation shape: {initialisation.shape}")

    Kp = torch.tensor([2.01, 2.01, 2.01], dtype=torch.float32).repeat(repeats)
    Ki = torch.tensor([0.108, 0.108, 0.108], dtype=torch.float32).repeat(repeats)
    controller_params = generate_controller_params_torch(Kp, Ki)
    print(f"Controller params shape: {controller_params.shape}")
    print(f"Controller params: {controller_params}")

    params_bis = generate_Vdc_bool_Transducer_torch(B)
    print(f"Params bis shape: {params_bis.shape}")

    R_down = torch.tensor([1.0, 1.0, 1.0], dtype=torch.float32).repeat(repeats)
    R_up = torch.tensor([7.0, 7.0, 7.0], dtype=torch.float32).repeat(repeats)
    t_R = torch.tensor([5e-3, 5e-3, 5e-3], dtype=torch.float32).repeat(repeats)
    Ls = torch.tensor([1.2e-3, 1.2e-3, 1.2e-3], dtype=torch.float32).repeat(repeats)
    params =  generate_Rs_Ls_Es_torch(R_down, R_up, t_R, Ls)
    print(f"Params shape: {params.shape}")

    
    fig = plt.figure(figsize=(12, 8))
    for i in range(B):
        plt.plot(params[i, 0].cpu().numpy(), label=f"R {i+1}")
    plt.title(f"R")
    plt.xlabel("Time Steps (20kHz)")
    plt.legend()
    plt.grid()
    ...
    fig = plt.figure(figsize=(12, 8))
    for i in range(B):
        plt.plot(params[i, 2].cpu().numpy(), label=f"R {i+1}")
    plt.title(f"R")
    plt.xlabel("Time Steps (20kHz)")
    plt.legend()
    plt.grid()
    ...
    Iref = torch.tensor([5.0, 8.0, 10.0], dtype=torch.float32).repeat(repeats)
    input_sequence = generate_input_sequence_torch(Iref)
    print(f"Input sequence shape: {input_sequence.shape}")
    
    fig = plt.figure(figsize=(12, 8))
    for i in range(B):
        plt.plot(input_sequence[i].cpu().numpy(), label=f"R {i+1}")
    plt.title(f"R")
    plt.xlabel("Time Steps (20kHz)")
    plt.legend()
    plt.grid()
    ...
    params_bis = params_bis.to(device)
    controller_params = controller_params.to(device)
    initialisation = initialisation.to(device)

    import time
    start_time = time.time()
    print(f"start time: {start_time}", flush=True)
    output_sequence, final_initialization, final_controller_params = system_torch(input_sequence, params, params_bis, controller_params, initialisation)
    end_time = time.time()
    print(f"Execution time: {end_time - start_time}", flush=True)
    ...
    for i in range(B):
        plt.plot(input_sequence[i].cpu().numpy(), label=f"R {i+1}")
        plt.plot(output_sequence[i].cpu().numpy(), label=f"Output {i+1}")
    plt.title(f"R")
    plt.xlabel("Time Steps (20kHz)")
    plt.legend()
    plt.grid()
    plt.show()
    """
    #[TODO] missing: returns the last state of the controller (integral error and past error) for the next period, to be able to simulate multiple periods with state reset between them
    ...
    error = torch.tensor([[0.1, 0.2, 0.3], [0.1, 0.2, 0.3]], dtype=torch.float32).to(device) # shape (B, T)
    Kp = torch.tensor([2.0, 2.0], dtype=torch.float32).to(device) # shape (B,)
    Ki = torch.tensor([0.1, 0.1], dtype=torch.float32).to(device) # shape (B,)
    window = torch.tensor([0, 2], dtype=torch.int64).to(device) # shape (B,)

    out = reference_generator(error, Kp, Ki, window)
    print(f"Output shape: {out.shape}")
    print(f"Output: {out}")

    s = torch.tensor([[1.0, 2.0, 3.0], [3.6, 3.7, 3.8]], dtype=torch.float32).to(device) # shape (B, T)
    interp = Interpolate(s)
    print(f"Interpolated shape: {interp.shape}")
    ...
    """
    Iref = torch.as_tensor(I_refs, dtype=torch.float32).flatten().to(device) # shape (1)
    Kp = torch.tensor([Kps], dtype=torch.float32).to(device) # shape (1,)
    Ki = torch.tensor([Kis], dtype=torch.float32).to(device) # shape (1,)
    R_down = torch.tensor([Rs_down], dtype=torch.float32).to(device) # shape (1,)
    R_up = torch.tensor([Rs_up], dtype=torch.float32).to(device) # shape (1,)
    t_R = torch.tensor([t_Rs], dtype=torch.float32).to(device) # shape (1,)
    L = torch.tensor([Ls], dtype=torch.float32).to(device) # shape (1,)

    sample_Kps = torch.tensor([sample_Kps], dtype=torch.float32).to(device).repeat(10000) # shape (10000,)
    sample_Kis = torch.tensor([sample_Kis], dtype=torch.float32).to(device).repeat(10000) # shape (10000,)
    sample_windows = torch.tensor([sample_windows], dtype=torch.int64).to(device).repeat(10000) # shape (10000,)
    B_size = sample_Kps.size(0)

    input_sequence = generate_input_sequence_torch(Iref).to(device)
    initialisation = generate_initialization_torch(1).to(device)
    print(f"Initialisation shape: {initialisation.shape}")
    controller_params = generate_controller_params_torch(Kp, Ki)
    params_bis = generate_Vdc_bool_Transducer_torch(1).to(device)
    print(f"Params bis shape: {params_bis.shape}")
    ...
    import time
    start_time = time.time()
    output_sequence, final_initialisation, final_controller_params = system_torch(input_sequence, params, params_bis, controller_params, initialisation)
    end_time = time.time()
    print(f"Execution time: {end_time - start_time} seconds")
    print(f"Output sequence shape: {output_sequence.shape}")

    input_steps = torch.arange(input_sequence.size(1), device=device) #shape (T,)
    ...
    burst_starts = torch.randint(0, T, (B, MAX_BURST), device=device) #need to cycle here too
    ...
    print(f"Burst sequence shape: {burst_sequence.shape}")

    start_time = time.time()
    input_sequence = input_sequence.repeat(B_size, 1) #shape (B, T)
    error = Error(output_sequence, Iref, shift=True).repeat(B_size, 1) #shape (B, T)
    input_sequence_bis = input_sequence + reference_generator(error, sample_Kps, sample_Kis, sample_windows) + burst_sequence #shape (B, T)
    
    #Goal : add to input sequence the burst noise
    

    first_output_sequence = output_sequence

    initialisation = final_initialisation.repeat(B_size, 1) #shape (B, 3)
    controller_params = final_controller_params.repeat(B_size, 1) #shape (B, 6)
    params = params.repeat(B_size, 1, 1) #shape (B, 3, Clock_step)
    params_bis = params_bis.repeat(B_size, 1) #shape (B, 3)

    second_output_sequence, final_initialisation, final_controller_params = system_torch(input_sequence_bis, params, params_bis, controller_params, initialisation)#shape (B, T)
    end_time = time.time()
    print(f"Execution time: {end_time - start_time} seconds")
    print(f"Second output sequence shape: {second_output_sequence.shape}")
    
    B_plot = min(5, input_sequence.shape[0])
    fig, axes = plt.subplots(B_plot, 1, figsize=(12, 4 * B_plot), sharex=True)
    if B_plot == 1:
        axes = [axes]

    for b in range(B_plot):
        combined_reference = torch.cat((input_sequence[b], input_sequence_bis[b])).cpu().numpy()
        combined_output = torch.cat((first_output_sequence[0], second_output_sequence[b])).cpu().numpy()
        time_axis = np.arange(combined_reference.size)

        axes[b].plot(time_axis, combined_reference, label="Reference")
        axes[b].plot(time_axis, combined_output, label="Output")
        axes[b].axvline(input_sequence.shape[1] - 1, color="gray", linestyle="--", linewidth=1)
        axes[b].set_title(f"Batch {b}: Reference and Output over Two Consecutive Periods")
    ...
    """
    half_chunk_paths = chunk_paths[:81]
    if len(half_chunk_paths) < 81:
        raise ValueError(f"Expected at least 81 chunks to build output_half.pt, got {len(half_chunk_paths)}")

    half_input_sequence_bis = []
    half_sample_output = []
    for chunk_path in half_chunk_paths:
        chunk_input, chunk_output = torch.load(chunk_path, map_location="cpu")
        half_input_sequence_bis.append(chunk_input)
        half_sample_output.append(chunk_output)
    ...
    torch.save([merged_half_input_sequence_bis, merged_half_sample_output], f"dataset{j}/output_half.pt")
    print(f"Saved merged dataset to dataset{j}/output_half.pt with input shape {merged_half_input_sequence_bis.shape} and output shape {merged_half_sample_output.shape}", flush=True)
    """
